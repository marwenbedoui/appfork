...
def datasetGeneration(test_collection, rapports_collection):
    ...
    headers = ['cpuCapacity', 'memoryCapacity', 'requestNumber', 'added_lines', 'removed_lines', 'loops_add',
    ...
    with open('rapports.csv', 'w', newline='') as file:
        ...
        for test in tests:
            ...
            if len(rapports) > 0:
                ...
                processTime = 0, 0, 0
            writer.writerow([cpuCapacity, memoryCapacity, requestNumber, added_lines, removed_lines, loops_add,
                            loops_remove, conditions_add, conditions_remove, processTime, success])
...
def prediction(requestNumber, added_lines, removed_lines, loops_add, loops_remove, conditions_add, conditions_remove):
    ...
    X_train, X_test, y_train_success, y_test_success, y_train_processTime, y_test_processTime = train_test_split(
        X, y_success, y_processTime, test_size=0.2, random_state=42)
